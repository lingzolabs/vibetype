...
- **适合中英混说**：后端会统一做标点清理和确定性文本后处理；含中日韩文字时优先使用全角标点，纯英文等场景使用半角标点。
- **确定性文本处理**：不依赖 LLM，在后端统一完成 URL/邮箱/路径/版本号/代码片段保护、口语填充词清理、相邻重复短语去重、高置信改口修复、专有名词别名纠正（GitHub/JSON-RPC/Kubernetes 等）。
...
Vibetype 后端内置确定性文本处理流水线，不依赖 LLM，保证 CLI/IBus/Fcitx5 前端输出一致：

### partial（实时预览）
- 沿用后端原有的标点及空格规范化，不执行新增的口语清理和术语纠正规则
- URL/邮箱/文件路径/版本号/内联代码/技术标识符内部逐字符保持不变

### final（最终提交）
1. 受保护片段检测（URL、邮箱、文件路径、CLI 选项、版本号、反引号代码、含点技术标识符）
2. 标点规范化（不修改受保护片段）
3. 口语填充词清理：句首或标点边界的“嗯”/“呃”/“um”/“uh”
4. 配置白名单内的相邻重复去重，例如“今天今天”“然后然后”和“the the”；不对白名单外的叠词做猜测
5. 高置信日期/数字/版本模式改口：“周四，不对，周五” → “周五”
6. 专有名词别名纠正：最长匹配优先、Latin 词边界、CJK 连续匹配，受保护片段内不替换

### 配置分层
- 内置配置：`data/text-processing.json` + `data/computer_terms.json`
- 用户视需覆盖：`~/.config/vibetype/text-processing.json`（只覆盖显式字段）
- 第一版在后端启动时加载；修改后重启 `vibetype-backend` 用户服务
- 解析失败保守使用默认配置，不影响后端启动
