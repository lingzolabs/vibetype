...
- 前端：IBus + Python（MVP 基线）；当前仓库额外提供 Fcitx5 前端实现。
...
- MVP 基线设计不依赖 Fcitx5 前端；后端协议必须可被 Fcitx5 复用。当前仓库已提供 Fcitx5 前端实现。
...
Fcitx5 前端与 IBus/CLI 共用同一套 JSON-RPC 2.0 client 协议，不应要求修改后端。
